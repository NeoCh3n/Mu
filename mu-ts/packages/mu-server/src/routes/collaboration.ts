...
export function registerCollaborationRoutes(app: FastifyInstance, ctx: RouteContext): void {
  ...
  // Native clients use the Project/Space id generated by their local store.
  // Keeping this operation idempotent lets Swift and the TS client converge
  // on the same room without first creating a random server-side id.
  app.put('/spaces/:spaceID', async (
    request: FastifyRequest<{ Params: SpaceParams; Body: EnsureSpaceBody }>,
    reply: FastifyReply,
  ) => {
    const body = request.body ?? {}
    const spaceID = spaceIDFrom(request.params)
    const current = collaboration.fetchSpace(spaceID)
    if (current !== undefined) {
      return { space: current, created: false }
    }
    const displayName = typeof body.displayName === 'string' && body.displayName.trim() !== ''
      ? body.displayName
      : `Space ${spaceID.slice(0, 8)}`
    const space = collaboration.ensureSpace({
      id: spaceID,
      displayName,
      description: body.description,
      createdByActorID: principalFor(request).actorID,
    })
    return reply.status(201).send({ space, created: true })
  })
